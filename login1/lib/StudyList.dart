                  ...
                  child: Column(
                    children: [
                      Container(
                        ...
                        child: Column(
                          children: [
                            Text('과목명: TEST', style: TextStyle(
                                fontWeight: FontWeight.bold,
                                fontSize: 17
                            )),
                            Text('참여인원: 11/45 (명)', style: TextStyle(
                                fontWeight: FontWeight.bold,
                                fontSize: 17
                            )),
                          ],
                      ...
                      ListView.separated(
                          itemBuilder: (context, index) {
                            return GestureDetector(
                              onTap: () async { // 탭하면 위에 과목명, 참여인원 출력
                                List<String> studyList = [];

                                await db.collection('users').doc(uid!).get().then((user) {
                                  studyList = user.data()!['participatingStudyGroup'];
                                });
                                ...
                                child: Column(
                                  children: [
                                    Row(
                                        ...
                                        children: [
                                          Text('과목명 : ${studyRooms[index].subjectName}',
                                              style: TextStyle(
                                                  fontWeight: FontWeight.bold,
                                                  fontSize: 17)),
                                          Text('참여인원: ${studyRooms[index].members}/${studyRooms[index].memberLimit} (명)',
                                              style: TextStyle(
                                                  fontWeight: FontWeight.bold,
                                                  fontSize: 17))
                                        ]),
                                    ...
                                    Container(
                                      ...
                                      decoration: BoxDecoration(
                                          color: Colors.white,
                                          borderRadius: BorderRadius.circular(10)),
                                      child: Text('${studyRooms[index].studyIntro}',
                          ...
                          separatorBuilder: (BuildContext context, int index) {
                            return const Padding(padding: EdgeInsets.only(top: 15));
                          },
